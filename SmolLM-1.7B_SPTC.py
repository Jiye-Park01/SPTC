...
# 평가 함수
def evaluate(questions, answers, prompt_func):
    correct = 0
...
# 실험 실행
print("=== SmolLM-1.7B 실험 시작 ===")

print("▶ No-CoT 실험 중...")
no_cot_accuracy = evaluate(questions, answers, no_cot_prompt)
print(f"No-CoT Accuracy: {no_cot_accuracy:.2f}%")

print("▶ CoT (Few-shot, Step-by-Step) 실험 중...")
cot_accuracy = evaluate(questions, answers, cot_prompt)
print(f"CoT Accuracy: {cot_accuracy:.2f}%")

print("▶ ToT (Few-shot, 5-branches Tree-of-Thought) 실험 중...")
tot_accuracy = evaluate(questions, answers, tot_prompt)
print(f"ToT Accuracy: {tot_accuracy:.2f}%")
