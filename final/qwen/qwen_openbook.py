...
# 1. 모델 이름
phi2 = "Qwen/Qwen-1_8B-Chat"

# 2. 토크나이저 먼저 로딩
tokenizer = AutoTokenizer.from_pretrained(phi2, trust_remote_code=True)
embedder = SentenceTransformer('all-MiniLM-L6-v2')


# 3. Qwen은 pad_token을 추가할 수 없으므로 eos_token을 재사용
tokenizer.pad_token = tokenizer.eos_token
tokenizer.pad_token_id = tokenizer.eos_token_id

# 4. 모델 로딩 (tokenizer 이후에 해야 오류 없음)
